    ...
    pipeline {
        ...
        stages {
            ...
            stage('deploy staging'){
                steps {
                    echo 'deploy staging................................'
                    sh "port=\$(docker inspect --format='{{range \$p, \$conf := .Config.ExposedPorts}} {{\$p}} {{end}}' ${pipelineParams.NAME}:build-${BUILD_NUMBER} | cut -f1 -d\"/\") && docker run -d --rm -p \$(echo \$port):\$(echo \$port) ${pipelineParams.NAME}:build-${BUILD_NUMBER}"
                }
            }
        }
        post {
